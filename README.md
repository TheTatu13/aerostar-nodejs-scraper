...
|---|---|
| `AEROSTAR SA` | legal name, uppercase (e.g. `EXAMPLE COMPANY SRL`) |
| `AEROSTAR` | commercial brand |
| `950531` | fiscal code (CUI), no `RO` prefix |
| `https://www.aerostar.com` | `https://www.example.com` |
| `https://cariere.aerostar.ro/jobs` | the open-positions listing page |
| `` | the job sitemap URL (or `""` if the site has none) |
| `https://cariere.aerostar.ro/jobs/` | canonical job-permalink prefix, e.g. `https://www.example.com/jobs/` |
| `Bacau` | HQ city (falls back to `România` in the transform) |
| `div.layout-card_layout_card__f6TgL` / `p` / `` | primary CSS selectors for the listing (keep the generic fallbacks after them) |
| `TheTatu13` / `aerostar-nodejs-scraper` | the derived repo's owner / name |
